...
export class WatchService {
  ...
  async episodes(popular: boolean, requestId: string): Promise<ServiceResponse<WatchEpisodeEntry[]>> {
    ...
    return withCache({ cache: this.cache, locks: this.locks }, cacheKey, this.config.catalogTtlSeconds, WATCH_PARSER_VERSION, () => this.catalog.get<WatchEpisodeEntry[]>(cacheKey), async () => {
  ...
  async promos(popular: boolean, requestId: string): Promise<ServiceResponse<WatchPromoEntry[]>> {
    ...
    return withCache({ cache: this.cache, locks: this.locks }, cacheKey, this.config.catalogTtlSeconds, WATCH_PARSER_VERSION, () => this.catalog.get<WatchPromoEntry[]>(cacheKey), async () => {
