...
export class ClubService {
  ...
  constructor(private readonly db: D1Database, private readonly config: RuntimeConfig, source?: CatalogSource, waitUntil?: WaitUntil) {
    this.cache = new CacheRepository(db); this.locks = new RefreshLockRepository(db); this.deps = { cache: this.cache, locks: this.locks, waitUntil }; this.clubs = new ClubRepository(db); this.catalog = new CatalogListRepository(db); this.source = source ?? new MalClient(config);
  }
  ...
  async detail(rawId: string, requestId: string): Promise<ServiceResponse<ClubDetail>> {
    ...
    return withCache(this.deps, `club:${malId}:detail`, this.config.animeTtlSeconds, CLUB_PARSER_VERSION, () => this.clubs.get(malId), async () => {
  ...
  list(page: number, requestId: string): Promise<ServiceResponse<ClubListEntry[]>> {
    ...
    return withCache(this.deps, cacheKey, this.config.catalogTtlSeconds, CLUB_LIST_PARSER_VERSION, () => this.catalog.get<ClubListEntry[]>(cacheKey), async () => {
  ...
  relations(rawId: string, requestId: string): Promise<ServiceResponse<ClubRelations>> {
    ...
    return withCache(this.deps, cacheKey, this.config.animeTtlSeconds, CLUB_RELATIONS_PARSER_VERSION, () => this.catalog.get<ClubRelations>(cacheKey), async () => {
      const source = await this.source.getHtml(clubDetailUrl(malId), ['Club Stats']);
      if (source.kind !== 'success') throw sourceError(source);
  ...
  members(rawId: string, page: number, requestId: string): Promise<ServiceResponse<ClubMember[]>> {
    ...
    return withCache(this.deps, cacheKey, this.config.animeTtlSeconds, CLUB_MEMBERS_PARSER_VERSION, () => this.catalog.get<ClubMember[]>(cacheKey), async () => {
