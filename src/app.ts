...
for (const kind of ['anime', 'manga', 'characters', 'people'] as RandomKind[]) app.get(`/v1/random/${kind}`, async (c) => {
  try {
    ...
    return c.json(success(picked.data, { cached: true, stale: isStale(picked.fetchedAt, configFrom(c.env).catalogTtlSeconds), refreshFailed: false, fetchedAt: picked.fetchedAt }));
  }
  catch (error) { return errorResponse(c, error, c.get('requestId')); }
});
app.get('/v1/random/users', async (c) => {
  try {
    ...
    cacheHeader(c, result);
    c.header('Cache-Control', NO_STORE);
    return c.json(success(result.data, { cached: result.cached, stale: result.stale, refreshFailed: result.refreshFailed, fetchedAt: result.fetchedAt }));
  }
  catch (error) { return errorResponse(c, error, c.get('requestId')); }
});
