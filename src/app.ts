...
import { cacheControlFor, etagFor, isStale, matchesEtag, NO_STORE } from './http/caching';
...
function service(c: Context<{ Bindings: Env; Variables: Variables }>): UserService {
...
function mangaService(c: Context<{ Bindings: Env; Variables: Variables }>): MangaService {
  return new MangaService(c.env.DB, configFrom(c.env), undefined, background(c));
}
function characterService(c: Context<{ Bindings: Env; Variables: Variables }>): CharacterService {
  return new CharacterService(c.env.DB, configFrom(c.env), undefined, background(c));
}
function producerService(c: Context<{ Bindings: Env; Variables: Variables }>): ProducerService {
  return new ProducerService(c.env.DB, configFrom(c.env), undefined, background(c));
}
function clubService(c: Context<{ Bindings: Env; Variables: Variables }>): ClubService {
  return new ClubService(c.env.DB, configFrom(c.env), undefined, background(c));
}
function personService(c: Context<{ Bindings: Env; Variables: Variables }>): PersonService {
  return new PersonService(c.env.DB, configFrom(c.env), undefined, background(c));
}
function watchService(c: Context<{ Bindings: Env; Variables: Variables }>): WatchService {
  return new WatchService(c.env.DB, configFrom(c.env), undefined, background(c));
}
function recommendationService(c: Context<{ Bindings: Env; Variables: Variables }>): RecommendationService {
  return new RecommendationService(c.env.DB, configFrom(c.env), undefined, background(c));
}
function reviewService(c: Context<{ Bindings: Env; Variables: Variables }>): ReviewService {
  return new ReviewService(c.env.DB, configFrom(c.env), undefined, background(c));
}
function searchService(c: Context<{ Bindings: Env; Variables: Variables }>): SearchService {
