...
export class ClubRepository {
  ...
  async get(malId: number): Promise<ClubDetail | null> {
    const row = await this.db.prepare('SELECT payload_json FROM clubs WHERE mal_id=?').bind(malId).first<{ payload_json: string }>();
  ...
  async put(detail: ClubDetail, fetchedAt: string, version: string): Promise<void> {
    await this.db.prepare(`INSERT INTO clubs(mal_id, title, payload_json, fetched_at, parser_version)
      ...
      .bind(detail.malId, detail.title, JSON.stringify(detail), fetchedAt, version).run();
  }
}
