...
export class MangaRepository {
  ...
  async get(malId: number): Promise<MangaDetail | null> {
    const row = await this.db.prepare('SELECT payload_json FROM manga WHERE mal_id=?').bind(malId).first<{ payload_json: string }>();
  ...
  async put(detail: MangaDetail, fetchedAt: string, version: string): Promise<void> {
    await this.db.prepare(`INSERT INTO manga(mal_id, title, payload_json, fetched_at, parser_version)
