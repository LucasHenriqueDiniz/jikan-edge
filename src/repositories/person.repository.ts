...
export class PersonRepository {
  ...
  async get(malId: number): Promise<PersonDetail | null> {
    const row = await this.db.prepare('SELECT payload_json FROM people WHERE mal_id=?').bind(malId).first<{ payload_json: string }>();
  ...
  async put(detail: PersonDetail, fetchedAt: string, version: string): Promise<void> {
    await this.db.prepare(`INSERT INTO people(mal_id, name, payload_json, fetched_at, parser_version)
