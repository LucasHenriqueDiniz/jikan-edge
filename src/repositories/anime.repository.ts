...
export class AnimeRepository {
  ...
  async get(malId: number): Promise<AnimeDetail | null> {
    const row = await this.db.prepare('SELECT payload_json FROM anime WHERE mal_id=?').bind(malId).first<{ payload_json: string }>();
  ...
  async put(detail: AnimeDetail, fetchedAt: string, version: string): Promise<void> {
    await this.db.prepare(`INSERT INTO anime(mal_id, title, payload_json, fetched_at, parser_version)
