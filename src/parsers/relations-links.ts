...
export function extractRelations(html: string): RelationEntry[] {
  ...
  for (const block of html.split('class="entry borderClass').slice(1)) {
    ...
    const linkMatch = block.match(/class="title">\s*<a href="https:\/\/myanimelist\.net\/(anime|manga)\/(\d+)\/[^"]*">\s*([^<]+?)\s*<\/a>/i);
    if (!relation || !linkMatch) continue;
    relations.push({ relation, type: linkMatch[1] === 'manga' ? 'manga' : 'anime', malId: Number(linkMatch[2]), title: decodeHtml(linkMatch[3]) });
  }
  return relations;
}
...
export function extractExternalLinks(html: string): ExternalLink[] {
  ...
  for (const match of html.matchAll(/<a href="([^"]+)" target="_blank" class="link ga-click" data-ga-click-type="external-links-[^"]*"[^>]*>(?:<i[^>]*>\s*<\/i>|<img[^>]*>)<div class="caption">([^<]+)<\/div><\/a>/gi)) {
...
export function extractStreaming(html: string): StreamingEntry[] {
  ...
  try {
    ...
    return (parsed.data ?? [])
      .filter((item): item is { platform: { name: string }; available: boolean; url: string } => typeof item.platform?.name === 'string' && typeof item.url === 'string')
