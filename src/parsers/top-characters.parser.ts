...
const entrySchema = z.object({
  ...
  animeography: z.array(workRefSchema), mangaography: z.array(workRefSchema), favorites: z.number().nullable(),
});
...
export function parseTopCharacters(html: string): TopCharacterEntry[] {
  ...
  for (const match of html.matchAll(ROW_PATTERN)) {
    ...
    const parsed = entrySchema.safeParse({
      malId, name, nameKanji, imageUrl,
      animeography: extractWorkRefs(animeographyCell, 'anime'), mangaography: extractWorkRefs(mangaographyCell, 'manga'), favorites,
    });
