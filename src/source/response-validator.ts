...
export function classifyHtml(body: string, metadata: SourceMetadata, requiredMarkers: string[] = []): SourceResult<string> {
  ...
  if (!metadata.contentType?.toLowerCase().includes('text/html')) return { kind: 'suspicious', reason: 'unexpected_content_type', metadata };
  if (body.trim().length < 512) return { kind: 'suspicious', reason: 'document_too_small', metadata };
  if (suspiciousMarkers.some((marker) => marker.test(body))) return { kind: 'suspicious', reason: 'challenge_or_login', metadata };
  if (requiredMarkers.some((marker) => !body.includes(marker))) return { kind: 'suspicious', reason: 'required_structure_missing', metadata };
