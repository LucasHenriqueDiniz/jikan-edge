...
describe('genre taxonomy parser', () => {
  it('reads the full anime taxonomy with category and count', () => {
    ...
    expect(genres[0]).toEqual({ malId: 1, name: 'Action', url: 'https://myanimelist.net/anime/genre/1/Action', count: 5003, type: 'genres' });
    expect(genres.find((entry) => entry.malId === 12)).toEqual({ malId: 12, name: 'Hentai', url: 'https://myanimelist.net/anime/genre/12/Hentai', count: 1634, type: 'explicit_genres' });
    expect(genres.find((entry) => entry.malId === 27)).toMatchObject({ name: 'Shounen', type: 'demographics' });
    const byKind = genres.reduce<Record<string, number>>((acc, entry) => ({ ...acc, [entry.type]: (acc[entry.type] ?? 0) + 1 }), {});
  ...
  it('reads the manga taxonomy and points its urls at the manga tree', () => {
    ...
    expect(genres[0]).toEqual({ malId: 1, name: 'Action', url: 'https://myanimelist.net/manga/genre/1/Action', count: 11296, type: 'genres' });
  ...
  it('splits the count off a name that has parentheses of its own', () => {
    const genres = parseGenreTaxonomy(animeHtml, 'anime');
    expect(genres.find((entry) => entry.malId === 60)).toEqual({ malId: 60, name: 'Idols (Female)', url: 'https://myanimelist.net/anime/genre/60/Idols_(Female)', count: 417, type: 'themes' });
  });
