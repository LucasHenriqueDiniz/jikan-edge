...
function buildLargeFixture(count: number): string {
  const entries = Array.from({ length: count }, (_, index) => `<a href="/manga/magazine/${index + 1}/Mag_${index + 1}" class="genre-name-link">Mag ${index + 1} (${index + 1})</a>`).join('\n');
...
describe('magazines parser', () => {
  it('extracts a large magazine directory', () => {
    ...
    expect(magazines[0]).toEqual({ malId: 1, name: 'Mag 1', count: 1, url: 'https://myanimelist.net/manga/magazine/1/Mag_1' });
  });
