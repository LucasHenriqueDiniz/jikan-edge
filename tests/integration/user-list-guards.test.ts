...
const config: RuntimeConfig = { profileTtlSeconds: 60, listTtlSeconds: 60, animeTtlSeconds: 60, catalogTtlSeconds: 60, sourceTimeoutMs: 1_000, maxUpstreamBytes: 50_000_000, malUserAgent: 'test' };
...
function listPage(count: number, from: number): string {
  const items = Array.from({ length: count }, (_, index) => ({
    ...
    anime_id: from + index, anime_title: `Title ${from + index}`,
    anime_image_path: 'https://cdn.myanimelist.net/images/anime/1/1.jpg',
    start_date_string: null, finish_date_string: null, updated_at: 0,
  }));
...
function statistics(animeTotal: number): UserStatistics {
  return {
    anime: { watching: 0, completed: animeTotal, onHold: 0, dropped: 0, planToWatch: 0, totalEntries: animeTotal, rewatched: null, episodesWatched: null, daysWatched: null, meanScore: null },
    manga: { reading: 0, completed: 0, onHold: 0, dropped: 0, planToRead: 0, totalEntries: 0, reread: null, chaptersRead: null, volumesRead: null, daysRead: null, meanScore: null },
  };
}

const profile: UserProfile = { username: 'tester', canonicalUsername: 'tester', profileUrl: 'https://myanimelist.net/profile/tester', avatarUrl: null, about: null, gender: null, location: null, birthday: null, joinedAt: null, lastOnlineAt: null, fetchedAt };
...
describe('user list completeness guards', () => {
  it('refuses to store a prefix when the list runs past the page cap', async () => {
    ...
    const service = new UserService(bindings.DB, config, client((offset) => listPage(300, offset + 1)));
    await expect(service.mediaList('tester', 'anime', 'req', 1, 100)).rejects.toMatchObject({ code: 'LIST_TOO_LARGE', status: 501 });
  ...
  it('rejects a snapshot holding fewer entries than the profile declares', async () => {
    ...
    const service = new UserService(bindings.DB, config, client(() => listPage(273, 1)));
  ...
  it('stores the list when it matches the declared total', async () => {
    await new UserRepository(bindings.DB).saveProfile(profile, statistics(360));
    const service = new UserService(bindings.DB, config, client((offset) => (offset === 0 ? listPage(300, 1) : listPage(60, 301))));
  ...
  it('accepts a list larger than a stale declared total', async () => {
    ...
    const service = new UserService(bindings.DB, config, client(() => listPage(40, 1)));
    const result = await service.mediaList('tester', 'anime', 'req', 1, 100);
  ...
  it('serves the list when no statistics have been stored yet', async () => {
    const service = new UserService(bindings.DB, config, client(() => listPage(12, 1)));
    const result = await service.mediaList('tester', 'anime', 'req', 1, 100);
