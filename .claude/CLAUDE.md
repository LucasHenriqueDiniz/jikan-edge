...
- **Shape Jikan v4 no `/v1` (2026-07-28) — mudança de contrato.** O `/v1` era "Jikan mas camelCase"; agora as 92 rotas emitem o shape real do Jikan (`{pagination, data}` + nosso `meta` extra no topo, `mal_id`, `images.jpg.image_url`, `titles[]`, erro `{status,type,message,error}`). Migrar do Jikan virou trocar a base URL. A tradução vive **só na borda**, em `src/http/jikan/` (um módulo por grupo, mapeadores tipados contra as interfaces de domínio); `src/domain/` e `src/repositories/` seguem camelCase, então o `payload_json` do D1 não mudou. Não use transform recursivo de chaves — a conversão é reestruturação, e as chaves de `ScheduleByDay` são dados. `/v1/schedules` voltou a ser lista achatada (reverte a decisão de 2026-07-27); o dia vai em `broadcast.day`. Ver `docs/routes.md` e `docs/api.md`.
- **URLs de entidade não seguem todas o padrão `/{tipo}/{id}`**: clube é `clubs.php?cid=`, produtor é `/anime/producer/`, revista é `/manga/magazine/`. `malUrl()` delega esses três para `src/source/mal-urls.ts`. Dois bugs reais vieram daí (2026-07-28): clube 404ava e produtor apontava para um anime não relacionado. Ao adicionar tipo novo, confirme a URL real antes de assumir o padrão.
- **Toda rota `/v1` precisa de teste de contrato.** `tests/routes/*.contract.test.ts` (um por grupo) valida o shape Jikan e falha em qualquer chave camelCase sob `data`/`pagination`; `tests/routes/coverage.test.ts` quebra a suíte se um `app.get` novo não tiver teste. Ao adicionar rota, adicione o teste no arquivo do grupo.
